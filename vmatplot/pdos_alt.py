    ...
    try:
        ...
        if dos_total_data is not None:
            ...
            if len(energy_dos_shift) == len(pdos_dict["dos_shifted_energy"]):
                ...
                print(
                    "Warning: total DOS and projected DOS grids have different lengths; "
                    "interstitial was not calculated."
                )
        else:
...
def plot_single_pdos(title, matters_list=None, x_range=None, y_top=None):
    ...
    fermi_reference_pdos_data = None
    plotted_any = False
    for matter in matters_list:
        ...
        if pdos_data is None:
            ...
            continue
        if fermi_reference_pdos_data is None:
            fermi_reference_pdos_data = pdos_data
        ...
        plotted_any = True
    if not plotted_any:
        print("Warning: No PDoS curves were plotted. Check DOSCAR, POSCAR/CONTCAR, atom indices, and orbital names.")
    plt.axvline(x=0, linestyle="--", color=fermi_color[0], alpha=0.8, label="Fermi energy")
    if fermi_reference_pdos_data is not None:
        fermi_energy_text = f"Fermi energy\n({fermi_reference_pdos_data['efermi']:.3f} eV)"
