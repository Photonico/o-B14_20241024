...
### Visualization
def dielectric_systems_list(systems_list):
    # data = dielectric_systems_list(systems_list)
    ...
    if systems_list and not isinstance(systems_list[0], (list, tuple)):
        systems_list = [systems_list]
    systems = []
    for values_dir in systems_list:
        if len(values_dir) == 2:
...
# Mask both real and imaginary densities within the processed boundary
def mask_real_imag(energy_real, density_real, energy_imag, density_imag, y_boundary=None):
    lo, hi = process_boundary_alt(y_boundary)
    m = np.ones_like(density_real, bool)
    if lo is not None:
        m &= (density_real >= lo) & (density_imag >= lo)
    if hi is not None:
        m &= (density_real <= hi) & (density_imag <= hi)
    return energy_real[m], density_real[m], energy_imag[m], density_imag[m]

# Mask only real density
def mask_real(energy_real, density_real, y_boundary=None):
    lo, hi = process_boundary_alt(y_boundary)
    m = np.ones_like(density_real, bool)
    if lo is not None:
        m &= density_real >= lo
...
# Mask only imaginary density
def mask_imag(energy_imag, density_imag, y_boundary=None):
    lo, hi = process_boundary_alt(y_boundary)
    m = np.ones_like(density_imag, bool)
    if lo is not None:
        m &= density_imag >= lo
    if hi is not None:
        m &= density_imag <= hi
...
def plot_dielectric_function_backup(suptitle, systems=None, components=None,
                             layout="horizontal", expansion_label=True,
                             unit=None, x_boundary=(None, None), y_boundary=(None, None),
                             spectrum_flag=None, figure_size = (None,None)):
    ## Help information
...
def plot_dielectric_function(suptitle, systems=None, components=None,
    ...
    if expansion_flag is True:
        for subplot_index in range(2*len(components)):
            ...
            # ylabel
            if layout_flag == "vertical" and subplot_index%2 == 0:
                ax.set_ylabel("Dielectric function", fontsize=14+2*ncols)
            elif layout_flag == "horizontal" and subplot_index in range(2):
                ax.set_ylabel("Dielectric function", fontsize=14+2*ncols)
            # xlabel
            if layout_flag == "vertical" and subplot_index >= 2*len(components)-2:
                ax.set_xlabel(xaxis_label, fontsize=14+2*nrows)
            elif layout_flag == "horizontal" and subplot_index%2 == 1:
                ax.set_xlabel(xaxis_label, fontsize=14+2*nrows)
