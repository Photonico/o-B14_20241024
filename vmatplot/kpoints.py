...
def summarize_kpoints_free_energy(directory=".", lattice_boundary = None):
    result_file = "energy_kpoints.dat"
...
def plot_kpoints_free_energy_data(matter, source_data=None, kpoints_boundary=None, color_family="blue"):
    ...
    # Figure title
    plt.title(f"Energy versus kpoints {matter}")
    ...
    end_index = kpoints_sum.index(kpoints_end) if kpoints_end in kpoints_sum else len(kpoints_sum) - 1

    # Prepare kpoints configurations and energy values for plotting
    kpoints_plotting = sep_kpoints[start_index:end_index + 1]
