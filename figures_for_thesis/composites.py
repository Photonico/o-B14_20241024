...
ax,bz,pd,legend_ax=axes.flat
bz.axis('off');legend_ax.axis('off')
fig.subplots_adjust(left=.10,right=.99,bottom=.105,top=.96,hspace=.27,wspace=.18)
...
tab(pd,'(c) Orbital projections',loc='right');frame(pd)
handles,labels=pd.get_legend_handles_labels()
legend_ax.legend(handles,labels,loc='center',ncol=3,frameon=True,fancybox=True,
                  borderpad=.3,columnspacing=.8,handlelength=1.4)
...
for block in source.get_text('dict')['blocks']:
    for line in block.get('lines',[]):
        for span in line['spans']:
            ...
            bz.text(*span['origin'],span['text'],fontsize=11,color=color,va='baseline')
bz.text(122.5,143,r'$\Gamma$',fontsize=11,color='#C82864')
...
fig=plt.figure(figsize=(10,4.5));gs=fig.add_gridspec(2,2,width_ratios=[2.6,1],left=.12,right=.99,bottom=.14,top=.96,hspace=.13,wspace=.05)
